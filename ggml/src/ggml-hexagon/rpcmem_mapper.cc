...
void RpcMemMapper::validate(const ggml_tensor * dst) {
    ...
    auto add_buffer = [&](ggml_backend_buffer * buf) {
        if (ggml_backend_buft_is_hexagon_rpcmem(buf->buft)) {
...
again:
            ...
            if (err) {
                ...
                for (int i = 0; i < GGML_MAX_SRC; ++i) {
                    ...
                    if (ggml_backend_buft_is_hexagon_rpcmem(src->buffer->buft)) {
...
std::pair<int, ssize_t> RpcMemMapper::get_tensor_mapping(const ggml_tensor * tensor) const {
    GGML_ASSERT(ggml_backend_buft_is_hexagon_rpcmem(tensor->buffer->buft));
